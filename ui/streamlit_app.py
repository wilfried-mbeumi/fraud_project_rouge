...
try:
    ...
    st.markdown('<p class="status-err">● API inaccessible — vérifiez l\'URL Render dans streamlit_app.py</p>', unsafe_allow_html=True)
except requests.exceptions.Timeout:
    st.markdown('<p class="status-err">● Timeout — l\'instance Render est peut-être en veille (patientez 50s)</p>', unsafe_allow_html=True)
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            <div class="result-card {css}">
              ...
              <div class="result-meta">Modèle {res['model_version']} · ROC-AUC {res['roc_auc']*100:.1f}%</div>
            </div>
            """, unsafe_allow_html=True)
        elif r.status_code == 422:
            st.error("❌ Données invalides (Erreur 422) — vérifiez les champs")
        else:
            st.error(f"❌ Erreur API : {r.status_code}")
    except requests.exceptions.ConnectionError:
        st.error("❌ API inaccessible. Vérifiez l'URL Render dans streamlit_app.py")
    except requests.exceptions.Timeout:
        st.error("❌ Timeout — l'instance Render est en veille, patientez 50s et réessayez")
